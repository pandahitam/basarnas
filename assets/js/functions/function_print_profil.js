...
function showForm_Print(url_print,title_print,selected_val){
	var form_print = new Ext.create('Ext.form.Panel', {
  	id: 'frmprint_Profil', url: BASE_URL + 'print_checkselected', frame: true, bodyStyle: 'padding: 5px 5px 0 0;', width: '100%', height: '100%',
    ...
    buttons: [{
    	text: 'Preview',
      handler: function() {
				Ext.getCmp('frmprint_Profil').on({
    			beforeaction: function() {Ext.getCmp('winPrint_Profil').body.mask();Ext.getCmp('sbwinPrint').showBusy();}
    		});
        ...
        form_print.getForm().submit({            			
          ...
          failure: function(form, action){
          	Ext.getCmp('winPrint_Profil').body.unmask();
	...
	var win_popup_print = new Ext.create('Ext.window.Window', {
		id: 'winPrint_Profil', title: 'Cetak Profil Pegawai', iconCls: 'icon-printer', constrainHeader : true, closable: true,
...
function show_pdf_print(url_print, title_print, mode_cetak, dari, sampai, terpilih){
	var docprint = new Ext.create('Ext.window.Window', {
		...
		width: 600, height: 500, bodyStyle: 'padding: 5px;', modal : true,
		items: [{
