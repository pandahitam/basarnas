<?php
class Tasset_kendaraan_Model extends CI_Model {
	
	var $table;
	
	function __construct(){
		parent::__construct();
		$this->table = 'tasset_kendaraan';
	}
	...
	function get_AllData($start = NULL, $limit=NULL){
		...
		$this->db->select('id,nama_unker,kode_asset,kode_wilayah,kode_registrasi,kode_unit_kerja,jabatan_unor,kode_unit_organisasi,nama,asal_usul,keterangan,
						harga,merek,tipe,cc,warna,nomor_rangka,nomor_mesin,nomor_polisi,
						tahun_pembuatan,tahun_pembelian,bpkb_tanggal,bpkb_nomor,image_url');
	...
	function ConstructKode($kode_golongan = NULL,$kode_asset = NULL){
		...
		{
			$kode = '3' . $kode_golongan . $kode_asset;
		}
