...
class Asset_Angkutan_Detail_Penggunaan_Model extends MY_Model{
	...
	function getSpecificDetailPenggunaanAngkutan($id_ext_asset)
        ...
        function getSpecificDetailPenggunaanAngkutanUdara($id_ext_asset)
        {
            if($_POST['open'] == 1)
            {

                    $query = "select id,id_ext_asset,tanggal,jumlah_penggunaan,satuan_penggunaan,keterangan, jumlah_cycle 
                        FROM ext_asset_angkutan_udara_detail_penggunaan WHERE id_ext_asset = $id_ext_asset";
