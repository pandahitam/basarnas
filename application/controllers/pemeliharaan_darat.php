<?php
class Pemeliharaan_Darat extends MY_Controller {
	...
	function __construct() {
		...
		$this->load->model('Pemeliharaan_Darat_Model','',TRUE);
		$this->model = $this->Pemeliharaan_Darat_Model;		
	}
	
	function index(){
		if($this->input->post("id_open")){
			$data['jsscript'] = TRUE;
			$this->load->view('process_asset/pemeliharaan_darat_view',$data);
		}else{
			$this->load->view('process_asset/pemeliharaan_darat_view');
		}
	}
	
	function modifyPemeliharaanDarat(){
                $data = array();
...
//		$this->modifyData(null,$data);
                if($data['id'] == '')
                {
                    ...
                    $this->createLog('INSERT PEMELIHARAAN KENDARAAN DARAT','pemeliharaan');
                }
                else
                {
                    $id = $data['id'];
                    ...
                    $this->createLog('UPDATE PEMELIHARAAN KENDARAAN DARAT','pemeliharaan');
                }
                echo "{success:true, id:$id}";
	}
	
	function deletePemeliharaanDarat()
	{
		$data = $this->input->post('data');
                foreach($data as $dataContent)
                {
                    $this->createLog('DELETE PEMELIHARAAN KENDARAAN DARAT','pemeliharaan');
                }
		return $this->deleteProcess($data);
	}
	
	
	function getSpecificPemeliharaanDarat()
	{
