<?php
class Pemeliharaan_Laut extends MY_Controller {
	...
	function __construct() {
		...
		$this->load->model('Pemeliharaan_Laut_Model','',TRUE);
		$this->model = $this->Pemeliharaan_Laut_Model;		
	}
	
	function index(){
		if($this->input->post("id_open")){
			$data['jsscript'] = TRUE;
			$this->load->view('process_asset/pemeliharaan_laut_view',$data);
		}else{
			$this->load->view('process_asset/pemeliharaan_laut_view');
		}
	}
	
	function modifyPemeliharaanLaut(){
                $data = array();
...
//		$this->modifyData(null,$data);
                if($data['id'] == '')
                {
                    ...
                    $this->createLog('INSERT PEMELIHARAAN KENDARAAN LAUT','pemeliharaan');
                }
                else
                {
                    $id = $data['id'];
	...
	function deletePemeliharaanLaut()
	{
		$data = $this->input->post('data');
                ...
                {
                    $this->createLog('DELETE PEMELIHARAAN KENDARAAN LAUT','pemeliharaan');
                }
		return $this->deleteProcess($data);
	}
	
	
	function getSpecificPemeliharaanLaut()
	{
