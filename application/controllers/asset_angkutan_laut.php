...
//                die;
		...
		foreach ($simakFields as $field) {
			...
			$dataExt[$field] = $this->input->post($field);
                ...
                if($dataExt['id'] != '')
                {
                    $this->createLog('UPDATE ASSET ANGKUTAN LAUT','asset_angkutan,ext_asset_angkutan_laut');
                }
                else
                {
                    $this->createLog('INSERT ASSET ANGKUTAN LAUT','asset_angkutan,ext_asset_angkutan_laut');
                }
	...
	function deleteAngkutanLaut()
	{
		$data = $this->input->post('data');
                ...
                {
                    $this->createLog('DELETE ASSET ANGKUTAN LAUT','asset_angkutan,ext_asset_angkutan_laut');
                }
...
//                $dataSend['total'] = $total;
		...
		echo json_encode($dataSend);
         ...
         function getSpecificPerlengkapanAngkutanLautWithoutIdExtAsset()
        {
                $kd_brg = $_POST['kd_brg'];
                $kd_lokasi = $_POST['kd_lokasi'];
                $no_aset = $_POST['no_aset'];
                ...
                $data = $this->model->getSpecificPerlengkapanAngkutanLaut($queryIdExtAsset_result->id);
                //                $total = $this->model->get_CountData();
//                $dataSend['total'] = $total;
		$dataSend['results'] = $data;
		echo json_encode($dataSend);
        ...
        function modifyPerlengkapanAngkutanLaut()
        {
            ...
            if($dataPerlengkapanLaut['id_asset_perlengkapan'] != null  && $dataPerlengkapanLaut['id_asset_perlengkapan'] != 0)
            {
                ...
                $this->db->where('id',$dataPerlengkapanLaut['id_asset_perlengkapan']);
