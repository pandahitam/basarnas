...
//		$this->model = $this->Asset_Alatbesar_Model;		
        ...
        public function getLaporanChart()
        {
            ...
            else
            {
                ...
                {
                    $query = $this->db->query( "select  
                    (select (sum(rph_aset)) from asset_angkutan where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Angkutan',
                    (select (sum(rph_aset)) from asset_bangunan where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Bangunan',
                    (select (sum(rph_aset)) from ext_asset_dil where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Luar',
                    (select (sum(rph_aset)) from asset_perairan where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Perairan',
                    (select (sum(rph_aset)) from asset_alatbesar where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Peralatan',
                    (select (sum(rph_aset)) from ext_asset_ruang where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Ruang',
                    (select (sum(rph_aset)) from asset_senjata where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Senjata',
                    (select (sum(rph_aset)) from asset_tanah where kd_lokasi='".$kd_lokasi."' and YEAR(tgl_buku) ='".$tahun."') as 'Tanah'");
                    $result = $query->row_array();
                    $result_array = array();
                    foreach($result as $key=>$value)
                    {
