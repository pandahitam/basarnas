<?php
class Asset_Angkutan extends MY_Controller {
	...
	function __construct() {
		...
		$this->model = $this->Asset_Angkutan_Model;		
	}
	
	function angkutan(){
		if($this->input->post("id_open")){
			$data['jsscript'] = TRUE;
			$this->load->view('pengelolaan_asset/angkutan_view',$data);
		}else{
			$this->load->view('pengelolaan_asset/angkutan_view');
		}
	}
	...
	function modifyAngkutan(){
		$dataSimak = array();
	  	...
	  	$simakFields = array(
			...
			'rphwajar', 'status','kd_klasifikasi_aset'
                );
