...
<?php if (isset($jsscript) && $jsscript == TRUE) { ?>
    ...
    // TABEL UNIT KERJA  --------------------------------------------------------- START
        ...
        var Data_UK = new Ext.create('Ext.data.Store', {
            ...
            proxy: Proxy_UK, groupField: 'ur_upb'
        });
        ...
        var filters_UK = new Ext.create('Ext.ux.grid.filter.Filter', {
            ...
            filters: [
                {type: 'string', dataIndex: 'kdlok'},
                {type: 'string', dataIndex: 'ur_upb'},
            ]
        });

        var cbGrid_UK = new Ext.create('Ext.selection.CheckboxModel');

        // ADI : I REMOVE THE GROUPING_UK ON GRID FEATURES, TO ADD THE GROUP BACK SIMPLY ADD grouping_UK into Grid Features
        var Grid_UK = new Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {header: "Kode", dataIndex: 'kdlok', width: 200, groupable: false},
                {header: "Nama Unit Kerja", dataIndex: 'ur_upb', width: 350, groupable: false},
            ],
            features: [filters_UK],
            tbar: tb_UK,
            ...
            var form_UK = new Ext.create('Ext.form.Panel', {
                ...
                items: [
                    ...
                    {xtype: 'combo', fieldLabel: 'Induk', name: 'kode_uki', hiddenName: 'kode_uki', allowBlank: true,
                        ...
                        listeners: {'focus': {fn: function(comboField) {
                                    comboField.expand();
                                }, scope: this}}
                    },
                ...
                buttons: [
                    ...
                    {text: 'Tutup', id: 'Tutup_M_UK', iconCls: 'icon-cross', handler: function() {
                            popup_UK.close();
                        }}
                ]
            });
