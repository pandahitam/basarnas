...
<?php if (isset($jsscript) && $jsscript == TRUE) { ?>
    ...
    // TABEL UNIT ORGANISASI  --------------------------------------------------------- START
        ...
        var Data_Unor = new Ext.create('Ext.data.Store', {
            id: 'Data_Unor', model: 'MUnitOrganisasi', pageSize: 20, noCache: false, autoLoad: true,
            proxy: Proxy_Unor, groupField: 'ur_upb'
        });

        var grouping_Unor = Ext.create('Ext.grid.feature.Grouping', {
            groupHeaderTpl: 'Group By : {name} ({rows.length} Item{[values.rows.length > 1 ? "s on this group" : " on this page"]})'
        });
        ...
        var grid_Unor = new Ext.create('Ext.grid.Panel', {
            ...
            listeners: {
                itemdblclick: function(dataview, record, item, index, e) {
                    if (unor_update == false) {
                        Ext.getCmp('Btn_Ubah_M_Unor').handler.call(Ext.getCmp("Btn_Ubah_M_Unor").scope);
                    }
                }
            }
        });
            ...
            var Form_Unor = new Ext.create('Ext.form.Panel', {
                    ...
                    {xtype: 'fieldcontainer', fieldLabel: 'Nama Unit Kerja', combineErrors: false,
                        ...
                        items: [
                            ...
                            {xtype: 'button', name: 'search_unker_for_unor', text: '...',
                                handler: function() {
                                    Show_Form_Ref_UnKer_For_Unor(Form_Unor);
                                }
                            }
                        ], anchor: '100%'
                    },
                ...
                buttons: [
                    {text: 'Tambah', name: 'Tambah_M_Unor', id: 'Tambah_M_Unor', iconCls: 'icon-add',
                        handler: function() {
                            Form_Unor.getForm().reset();
                            Ext.getCmp('Tambah_M_Unor').setDisabled(true);
                            Ext.getCmp('Ubah_M_Unor').setDisabled(true);
                            Ext.getCmp('Simpan_M_Unor').setDisabled(false);
                            Ext.getCmp('Batal_M_Unor').setDisabled(false);
                            Ext.getCmp('nama_unor').focus(false, 200);
                            Ext.getCmp('sbWin_Unor').setStatus({text: 'Ready', iconCls: 'x-status-valid'});
                            Ext.Ajax.request({
                    ...
                    {text: 'Simpan', id: 'Simpan_M_Unor', iconCls: 'icon-save', handler: function() {
                            Ext.getCmp('Form_Unor').on({
                                beforeaction: function() {
                                    Ext.getCmp('sbWin_Unor').showBusy();
                                }
                            });
                    ...
                    {text: 'Batal', id: 'Batal_M_Unor', iconCls: 'icon-undo',
                        handler: function() {
                            ...
                            Ext.getCmp('Simpan_M_Unor').setDisabled(true);
                            Ext.getCmp('Batal_M_Unor').setDisabled(true);
                            Form_Unor.getForm().setValues({kode_jns_unker: val_kode_jns_unker});
                            Ext.getCmp('nama_unker').focus(false, 200);
                            Ext.getCmp('sbWin_Unor').setStatus({text: 'Ready', iconCls: 'x-status-valid'});
                        }
                    },
                    {text: 'Tutup', id: 'Tutup_M_Unor', iconCls: 'icon-cross', handler: function() {
                            win_popup_Unor.close();
                        }}
                ]
            });
